...
type CameraSetupBoxProps = {
  streams: string[] // expected length 4 (cam1..cam4)
}

export default function CameraSetupBox({ streams }: CameraSetupBoxProps) {
  const [activeCam, setActiveCam] = useState<CamId>("topLeft")
  const activeLabel = useMemo(() => camTitle(activeCam), [activeCam])

  const safeStreams = useMemo(() => {
    const next = Array.isArray(streams) ? streams.slice(0, 4) : []
  ...
  const activeStreamUrl = useMemo(() => {
    ...
    return safeStreams[idx] || ""
  }, [activeCam, safeStreams])
          ...
          <div className="cs-streamPanel">
            <div className="cs-streamGlass">
              {activeStreamUrl ? (
                ...
                >
                  No video for {activeLabel}
                </div>
              )}
              <div className="cs-streamOverlay">
                <div className="cs-streamTop">
                  <div className="cs-streamTitle">View</div>
